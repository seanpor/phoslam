...
Greene.calcs <- function(d1, AltQ=NULL) {
  ...
  Greene.A <- stats::coef(Greene)[['A']]
  Greene.B <- stats::coef(Greene)[['B']]
  Greene.C <- stats::coef(Greene)[['C']]

  d1$GreenePred <- stats::predict(Greene, Qsource)
  ...
  list(Greene=Greene,
       Greene.coefficients=stats::coef(Greene),
       Greene.Qe=Greene.Qe,
